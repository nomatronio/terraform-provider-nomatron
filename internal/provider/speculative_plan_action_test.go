...
func TestNewSpeculativePlanAction(t *testing.T) {
	t.Parallel()

	a := NewSpeculativePlanAction()
	if a == nil {
		t.Fatal("expected action to be non-nil")
	}

	if _, ok := a.(*SpeculativePlanAction); !ok {
		t.Fatalf("expected *SpeculativePlanAction, got %T", a)
	}
}

func TestSpeculativePlanAction_Metadata(t *testing.T) {
	t.Parallel()

	a := &SpeculativePlanAction{}
	req := action.MetadataRequest{ProviderTypeName: "nomatron"}
	resp := &action.MetadataResponse{}

	a.Metadata(context.Background(), req, resp)

	if resp.TypeName != "nomatron_speculative_plan" {
		t.Fatalf("expected type name %q, got %q", "nomatron_speculative_plan", resp.TypeName)
	}
}

func TestSpeculativePlanAction_Schema(t *testing.T) {
	t.Parallel()
	...
	assertActionStringAttribute(t, attrs, "ref", false, true)
}

func TestSpeculativePlanAction_ConfigureNilProviderData(t *testing.T) {
	t.Parallel()
...
func TestSpeculativePlanAction_ConfigureWrongType(t *testing.T) {
	t.Parallel()
...
func TestSpeculativePlanAction_ConfigureSetsClient(t *testing.T) {
	t.Parallel()
